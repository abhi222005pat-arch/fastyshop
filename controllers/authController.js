...
const sign = u => jwt.sign(
  ...
  process.env.JWT_SECRET || 'fastyshop_secret',
  { expiresIn: '7d' }
);
...
exports.register = async (req, res) => {
  try {
    ...
    const [r]  = await db.query('INSERT INTO users (name,email,password,phone) VALUES (?,?,?,?)', [name.trim(), email.trim(), hash, phone||null]);
...
exports.login = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Login successful!', token: sign(rows[0]), user });
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
};
...
exports.getProfile = async (req, res) => {
  try {
    const [rows] = await db.query('SELECT id,name,email,phone,address,city,pincode,role,created_at FROM users WHERE id=?', [req.user.id]);
...
exports.updateProfile = async (req, res) => {
  try {
    ...
    await db.query('UPDATE users SET name=?,phone=?,address=?,city=?,pincode=? WHERE id=?', [name,phone,address,city,pincode,req.user.id]);
